...
void QueryHNSW(_1D_vec_f& q, int k, int ep, _1D_vec_i& indptr, _1D_vec_i& index, _1D_vec_i& level_offset , int max_level, _2D_vec_f& vect, int *result) 
{
	minHeap top_k;
...
void vect_transfer(int num_vec, int num_attrs, int size, int rank, _2D_vec_f &vect)
{
	MPI_File in;
	MPI_Status status;
	MPI_Offset offset;
    ...
    MPI_File_open(MPI_COMM_WORLD, "bin_vect.dat", MPI_MODE_RDWR|MPI_MODE_CREATE, MPI_INFO_NULL, &in);
...
void user_transfer(int rank, int size, int num_users, int num_attrs, int &k, int &ep, int &max_level, _1D_vec_i &index, _1D_vec_i &indptr, _1D_vec_i &level, _1D_vec_i &level_offset, _2D_vec_f &user)
{
	ifstream file;
    ...
    MPI_File in;
    MPI_Status status;
    MPI_File_open(MPI_COMM_WORLD, "bin_user.dat", MPI_MODE_RDWR|MPI_MODE_CREATE, MPI_INFO_NULL, &in);
    MPI_File_set_view(in, 0, MPI_FLOAT, MPI_FLOAT, "native", MPI_INFO_NULL ) ; 
...
int main(int argc, char* argv[])
{
    ...
    _2D_vec_f vect;
    vect_transfer(num_vec, num_attrs, size, rank, vect);
	...
	_2D_vec_f user;
	user_transfer(rank, size, num_users, num_attrs, k, ep, max_level, index, indptr, level, level_offset, user);
	...
	#pragma omp parallel num_threads(C)
	{
		...
		{
			QueryHNSW(user[i], k, ep, indptr, index, level_offset , max_level, vect, result[i]);
		}
	}
